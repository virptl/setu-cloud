...
func (r *Router) HandleUp(_ pahomqtt.Client, msg pahomqtt.Message) {
	tid, _, did, _, ok := parseTopic(msg.Topic())
	if !ok {
		return
	}
...
func (r *Router) HandleShd(_ pahomqtt.Client, msg pahomqtt.Message) {
	...
	var shd shdMessage
	if err := json.Unmarshal(msg.Payload(), &shd); err != nil {
	...
	// Mark device online based on retained shadow freshness (within 5 min).
	online := time.Since(time.Unix(shd.T, 0)) < 5*time.Minute
	if err := r.registry.SetOnline(ctx, tid, did, online); err != nil {
		log.Printf("mqtt/shd set online tid=%s did=%s: %v", tid, did, err)
	}
}
