...
#   setu_dn.sh ota URL [SIG]      trigger OTA (SIG = 128-hex ES256 sig; required
#                                 unless firmware built -DSETU_OTA_INSECURE=1)
...
case "$1" in
  ...
  ota)    if [ -n "$3" ]; then
            pub "{\"v\":\"1\",\"id\":\"$ID\",\"c\":\"ota\",\"d\":{\"url\":\"$2\",\"sig\":\"$3\"}}"
          else
            pub "{\"v\":\"1\",\"id\":\"$ID\",\"c\":\"ota\",\"d\":{\"url\":\"$2\"}}"
          fi ;;
  ...
  *)      echo "usage: on|off|rgb R G B|bright N|temp N|get|ota URL [SIG]|cfg R G B [freq]|raw JSON|watch"; exit 1 ;;
esac
